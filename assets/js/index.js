class GeradorSenha {
    ...
    gerarSenha(caracteres = 8, numeros = false, maiusculas = false, minusculas = false, simbolos = false) {
        ...
        if (numeros) funcoesGeradoras.push(GeradorSenha.randNumero);
        if (maiusculas) funcoesGeradoras.push(GeradorSenha.randLetraMaiuscula);
        if (minusculas) funcoesGeradoras.push(GeradorSenha.randLetraMinuscula);
        if (simbolos) funcoesGeradoras.push(GeradorSenha.randSimbolo);

        const senhaArray = [];
        for (let i = 0; i < caracteres; i++) {
            const funcaoSorteada = funcoesGeradoras[GeradorSenha.rand(0, funcoesGeradoras.length)];
            senhaArray.push(funcaoSorteada());
        }
    ...
    cliqueBotaoGerar() {
        this.botaoGerar.addEventListener('click', () => {
            ...
            if (dados[0] !== '' && dados.includes(true)) { // Validação de campos - Passou
                ...
                this.adicionarErro('Selecione as opções');
            }
        })
    }
